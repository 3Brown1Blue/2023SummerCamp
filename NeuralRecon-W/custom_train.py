...
from tools.prepare_data.generate_voxel import convert_to_dense,gen_octree,octree_to_spc
...
# a new defined system
class LightNeuconWSystem(LightningModule):
    def __init__(self, hparams, config, caches):
        ...
        self.loss = loss_dict["lightneuconw"](**self.config.NEUCONW.LOSS, config=config)
        ...
        # define a custom NEUCONW model
        self.lightneuconw = LightNeuconW(
            sdfNet_config=self.config.NEUCONW.SDF_CONFIG,
            lightcodeNet_config=self.config.LIGHTNEUCONW.LIGHT_CONFIG,
...
def main(hparams,config):
    ...
    system = LightNeuconWSystem(hparams,config,caches)
    datamodule=DataModule(hparams, config)
    ...
    trainer = Trainer(max_epochs=hparams.num_epochs,
                      ...
                      replace_sampler_ddp=False if config.DATASET.DATASET_NAME=='phototourism' and config.DATASET.PHOTOTOURISM.IMG_DOWNSCALE==1
                      else True,
...
if __name__ == '__main__':
    ...
    #general config for all scenes
    config = get_cfg_defaults()

    #merge config for different scenes
